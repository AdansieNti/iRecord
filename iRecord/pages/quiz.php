...
// Handling form submission for quiz records
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($_POST['students'] as $student_id => $data) {
        ...
        $sql = "INSERT INTO quiz (student_id, class, school_year, semester, subject, total, grade, remark) 
                VALUES ('$student_id', '$class', '$school_year', '$semester', '$subject', '$total', '$grade', '$remark')";
        $conn->query($sql);
    }
...
<head>
    ...
    <script>
        function updateGrade(studentId) {
            const total = parseFloat(document.querySelector(`input[name="students[${studentId}][total]"]`).value) || 0;
...
<div class="form-container">
    <h2>Search Class and Add Quiz</h2>
    <form method="GET">
            ...
            <?php if (!empty($students)): ?>
                <?php foreach ($students as $student): ?>
                    <tr>
                        ...
                        <td><input type="number" name="students[<?php echo $student['student_id']; ?>][total]" min="0" max="100" oninput="updateGrade(<?php echo $student['student_id']; ?>)"></td>
